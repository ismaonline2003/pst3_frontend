...
const searchBarParameters = [
    ...
    { value: 'emisor', label: 'Emisor', type: "search"},
    { value: 'fecha', label: 'Fecha', type: "search"}
];
...
export default function Grabaciones({}) {
    ...
    const searchRecords = (searchVals) => {
        ...
        let url = `${consts.backend_base_url}/api/grabacion?limit=${searchVals.limit}`;
    ...
    return (
        <React.Fragment>
            <br /> 
            <StyledH1>Grabaciones</StyledH1>
            <br />
            <SearchBarReadOnly selectOptions={searchBarParameters} externalHandleSearchBtn={handleSearchBtn} crearRoute={"/dashboard/grabaciones/0"} searchSelectValues={searchSelectValues}/>
            <br />
                ...
                <TableContainer>
                    <Table sx={{ minWidth: 650 }} aria-label="Proyectos">
                        ...
                        {records.filter((item, index) => index < (page+1)*rowsPerPage && index >= (page)*rowsPerPage).map((row, index) => {
                                const recordDate = new Date(row.fecha_inicio);
                                return (
                                    <TableRow
                                        key={row.id}
                                        ...
                                        >
                                            ...
                                            <TableCell align="left">{recordDate.getDate()}/{recordDate.getMonth()+1}/{recordDate.getFullYear()}</TableCell>
                                            ...
                                            <TableCell align="left">
                                                <Link to={`/dashboard/grabaciones/${row.id}`}>
